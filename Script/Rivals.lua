...
LegitTab:CreateSlider("Prediction Multiplier", 0.1, 1, 0.1, function(value)
    ...
    print("Multiplicateur de prédiction réglé à: " .. value)
end)

-- Bouton pour activer/désactiver l'aimbot
LegitTab:CreateToggle("Activer l'Aimbot", false, function(state)
    aimbotEnabled = state
    if state then
        print("Aimbot activé")
    else
...
VisualTab:CreateToggle("Activer les Box ESP", false, function(state)
    if state then
        print("Box ESP activé") -- Logique à ajouter pour activer l'ESP
    else
        print("Box ESP désactivé") -- Logique à ajouter pour désactiver l'ESP
    end
end)

-- Slider pour la transparence de l'ESP
VisualTab:CreateSlider("Transparence ESP", 0, 1, 0.5, function(value)
    print("Transparence ESP réglée à: " .. value) -- Logique à ajouter pour changer la transparence de l'ESP
end)
